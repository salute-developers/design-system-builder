...
function NLQueryContent({ onSaved }: { onSaved: () => void }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch(`${VITE_DB_SERVICE_API}/admin/nl-query`, {
        method: 'POST',
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const res = await fetch(`${VITE_DB_SERVICE_API}/ds/saved-queries`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
...
function QueriesPage() {
  ...
  const executeSavedQuery = async (id: string) => {
    ...
    try {
      const res = await fetch(`${VITE_DB_SERVICE_API}/ds/saved-queries/${id}/run`);
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || 'Query failed');
      setQueryResult(data);
    } catch (err) {
  ...
  const handleDeleteSaved = async (e: React.MouseEvent, id: string) => {
    e.stopPropagation();
    try {
      const res = await fetch(`${VITE_DB_SERVICE_API}/ds/saved-queries/${id}`, { method: 'DELETE' });
      if (!res.ok) return;
