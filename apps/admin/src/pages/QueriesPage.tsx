...
function NLQueryContent({ onSaved }: { onSaved: () => void }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch(`${VITE_DB_SERVICE_API}/nl-query`, {
        method: 'POST',
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const res = await fetch(`${VITE_DB_SERVICE_API}/saved-queries`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
...
function QueriesPage() {
  ...
  useEffect(() => {
    Promise.all([
      fetch(`${VITE_DB_SERVICE_API}/queries`)
        .then((r) => {
  ...
  const executeQuery = async (id: string, params?: Record<string, string>) => {
    ...
    try {
      const qs = params
        ...
        : '';
      const res = await fetch(`${VITE_DB_SERVICE_API}/queries/${id}${qs}`);
      const data = await res.json();
  ...
  const executeSavedQuery = async (id: string) => {
    ...
    try {
      const res = await fetch(`${VITE_DB_SERVICE_API}/saved-queries/${id}/run`);
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || 'Query failed');
      setQueryResult(data);
    } catch (err) {
  ...
  const handleDeleteSaved = async (e: React.MouseEvent, id: string) => {
    e.stopPropagation();
    try {
      const res = await fetch(`${VITE_DB_SERVICE_API}/saved-queries/${id}`, { method: 'DELETE' });
      if (!res.ok) return;
