...
CLI `components push` SHALL upload the whole package in a single request to the design-system scoped import endpoint.

#### Scenario: Push отправляет один запрос

- **WHEN** CLI has converted the package
- **THEN** CLI MUST send `POST /api/projects/{projectId}/ds/design-systems/{designSystemId}/components:import`
