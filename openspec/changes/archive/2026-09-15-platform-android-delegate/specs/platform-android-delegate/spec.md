...
- **WHEN** `toolchain doctor` runs and `<gradlew> -p <workspace.workspaceDir> help --task generateComposeTheme`
  (or `generateViewTheme`, whichever the workspace's `.sdds` targets configure) exits with code `0`
...
- **WHEN** `gradlew` is absent, cannot be started, or the `help --task` check exits non-zero
