...
## Purpose
Определяет адаптер платформы iOS для CLI `dsbuilder`: какие capability он обслуживает, в какие вызовы `dsbuilder-ios` их переводит и как находит инструмент на машине.
