---
description: ADR по MCP серверу в frontend-kt
---
...
Решение должно разрабатываться в [frontend-kt](../../frontend-kt/). Сейчас в этом проекте есть
[cli](../../frontend-kt/cli/) - Kotlin Multiplatform приложение, которое является локальной точкой интеграции с
DS Builder:
...
В дальнейшем в `frontend-kt` могут появиться другие клиенты DS Builder:
...
Бизнес-логика [cli](../../frontend-kt/cli/) должна быть вынесена в общий прикладной слой Kotlin Multiplatform
внутри `frontend-kt`. Терминальный CLI остается тонкой оберткой для запуска сценариев из терминала.

MCP сервер реализуется как общая Kotlin Multiplatform библиотека `:mcp-server-core` поверх того же прикладного слоя и
два независимых launcher. Его смогут использовать:

- терминальный CLI через команду `dsbuilder mcp serve`;
- отдельный Node.js executable `dsbuilder-mcp`, собранный из Kotlin/JS и поставляемый как npm-пакет;
- desktop приложение;
- IDE-плагины;
...
> сценариям. MCP сервер не должен быть тонкой оберткой над shell-командами CLI.

Оба launcher запускают `:mcp-server-core` внутри своего процесса. `dsbuilder mcp serve` не запускает Node.js subprocess,
а `dsbuilder-mcp` не зависит от установленного основного CLI.

Для реализации MCP используется официальный `io.modelcontextprotocol:kotlin-sdk-server:0.10.0`. Версия Kotlin выбирается
по совместимости с SDK и frontend toolchain и проверяется сборкой для JVM, Kotlin/JS, macOS arm64 и macOS x64. Типы SDK
не выходят за границу `:mcp-server-core` и не используются в `core-*` или `feature-*`.
...
Целевая декомпозиция `frontend-kt`:
...
frontend-kt/
  ...
  feature-auth/
    сценарии login, status и logout пользовательской сессии

  mcp-server-core/
    MCP-инструменты, server lifecycle, преобразование результата сценария в MCP-ответ

  mcp-node/
    Kotlin/JS Node.js launcher, platform runtime и npm packaging
...
MCP сервер MVP работает только через stdio и может запускаться двумя равноправными способами:
...
CLI launcher собирается для JVM/macOS и запускает общий server core внутри процесса. Отдельный `:mcp-node` собирается
как Kotlin/JS executable для Node.js. Оба варианта имеют одинаковые `--api-url`, правила контекста, авторизации,
набор инструментов и MCP-контракт. HTTP/SSE transport и удаленный MCP server в MVP не входят.

Процесс работает до EOF stdin или сигнала завершения. Stdout зарезервирован только для MCP protocol messages; логи и
диагностика процесса направляются в stderr. Закрытие stdin, `SIGINT` и `SIGTERM` должны корректно закрывать MCP session,
HTTP client и файловые ресурсы.
...
в данное решение о локальной среде выполнения MCP в `frontend-kt`.
...
Если ответ инструмента требует нескольких backend чтений, их оркестрация принадлежит одному прикладному сценарию.
Например, проекция `component_config_get` читает конфигурацию и каталог токенов с одним разрешённым runtime;
MCP передаёт параметры и сериализует результат.
...
Первая версия содержит два инструмента только для чтения:

- `design_system_get_context` — возвращает разрешенный контекст без raw credential;
- `project_get_status` — проверяет доступность настроенных project/design system через общий прикладной сценарий.

Этого вертикального среза достаточно, чтобы проверить оба launcher, stdio lifecycle, `tools/list`, `tools/call`, общий
`ContextResolver`, приоритет учетных данных, token refresh и структурированные ошибки. Документация, `CodeBinding`, токены,
темы и компоненты добавляются следующими изменениями поверх стабильного runtime.
...
список созданных/измененных файлов и диагностику. Они ограничиваются разрешенной рабочей директорией и не могут выходить
за нее через абсолютные пути, `..` или symlink.

Публикация меняет состояние backend и должна возвращать `jobId`, статус приемочной валидации и ссылку на последующую
диагностику. Право на публикацию определяет только DS Builder API через project-scoped RBAC/scopes; MCP и CLI не вводят
локальные роли, allowlist инструментов или флаги `--allow-publish`.
...
- не принимает и не возвращает пароль через MCP tools;
- получает `401`, `403`, `404` и другие ошибки от бэкенда/gateway и возвращает их как структурированную диагностику.

Для backend-запросов CLI и MCP используют единый приоритет: project key, если он найден в runtime environment по ссылке
из project context, затем пользовательская access/refresh session. Fallback выполняется только при отсутствии project key.
Невалидный project key или недостаток его scopes не приводят к незаметному переключению actor на пользователя.

Отдельная авторизация между MCP client и локальным stdio server не вводится: границей доверия является локальный процесс,
который MCP client запускает на устройстве пользователя. Авторизация MCP по OAuth потребуется только при появлении
удаленного HTTP transport и не входит в это решение.
...
- не обходить RBAC backend локальными проверками и не скрывать инструменты на основании локально вычисленных ролей;
- ограничивать локальные файловые операции разрешенной рабочей директорией.

## Последствия

- MCP становится альтернативным интерфейсом к общим возможностям `frontend-kt`, а не отдельным бэкенд-сервисом и
...
- Существующее выделение бизнес-логики из `frontend-kt/cli` в общие Kotlin Multiplatform модули продолжается для
  сценариев, используемых MCP.
...
- MCP можно поставлять отдельно как Node.js executable `dsbuilder-mcp` и запускать из основного CLI командой
  `dsbuilder mcp serve` без запуска одного executable из другого.
- Web-клиент не запускает локальный MCP server; удаленный MCP server остается отдельным рассматриваемым направлением.

## Открытые вопросы

- Нужен ли в будущем удаленный MCP server для web-клиента и облачных агентских сценариев?
- Потребуется ли локальный HTTP transport для desktop/IDE после проверки stdio-интеграций?
- Какие группы предметных инструментов добавлять после первого вертикального среза: документацию и CodeBinding или
  темы, токены и компоненты?
