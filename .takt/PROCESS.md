...
Если provider TAKT — Codex, `tools/task` запускает внутреннего агента с отдельным
`CODEX_HOME` в `~/.cache/dsbuilder-takt-codex`. Каталог общий для обычного
checkout и всех worktree, но содержит только состояние внутренних TAKT-сессий.
Он должен быть заранее создан, авторизован командой `codex login` с этим
`CODEX_HOME` и добавлен в `sandbox_workspace_write.writable_roots` внешнего
Codex. Подробная настройка приведена в корневом `README.md`.

Перед созданием состояния нового запуска helper проверяет запись в каталог и
статус авторизации. Путь можно переопределить абсолютным значением
`DSBUILDER_TAKT_CODEX_HOME`. Настройка не применяется, если выбран другой
provider.
