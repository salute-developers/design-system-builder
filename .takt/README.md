...
Обычный `resume` использует сохранённый TAKT run, пока его workflow и HEAD
совместимы с checkout. После rebase или изменения workflow та же команда
автоматически запускает актуальный workflow поверх сохранённых незакоммиченных
изменений и передаёт ему отчёт предыдущего запуска.
