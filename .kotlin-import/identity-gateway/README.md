...
При необходимости можно создать [`.env.local.example`](/Users/alex/IdeaProjects/design-system-builder-kt/identity-gateway/.env.local.example) -> `identity-gateway/.env.local`. Корневой скрипт [start-local.sh](/Users/alex/IdeaProjects/design-system-builder-kt/start-local.sh) автоматически подхватит этот файл перед запуском compose и bootstrap Keycloak.
...
Для production добавлен корневой [docker-compose.prod.yml](/Users/alex/IdeaProjects/design-system-builder-kt/docker-compose.prod.yml), env-шаблон [`.env.prod.example`](/Users/alex/IdeaProjects/design-system-builder-kt/.env.prod.example), отдельный nginx-шаблон [nginx.prod.conf.template](/Users/alex/IdeaProjects/design-system-builder-kt/identity-gateway/gateway/nginx.prod.conf.template) и встроенный `keycloak` stack для сценария, когда все сервисы живут на одной машине.
