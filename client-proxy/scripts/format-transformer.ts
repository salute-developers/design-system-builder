...
// Types for Client Storage Format
...
class FormatTransformer {
  ...
  transformClientToBackend(clientData: ClientFormat): BackendFormat {
    ...
    components.forEach(component => {
      // This would need to be populated from the configs transformation
    });
        ...
        variation.styles.forEach((style: any) => {
          ...
          if (style.props) {
            style.props.forEach((prop: any) => {
              ...
              const tokenId = this.findTokenIdByName(api, prop.id);
