...
- новая платформа = модуль `platform-<toolchain>` с реализацией `PlatformDelegate` и одна строка
  ...
  Пример — `platform-ios`: делегат, локатор инструмента, установщик и свой Koin-module;
- установка инструмента — отдельный порт `ToolchainInstaller` и свой реестр в том же
  `di/PlatformDelegatesModule.kt`. Toolchain без установщика остаётся рабочим: его инструмент
  ставят руками, а `--tool` и переменные окружения продолжают работать;
