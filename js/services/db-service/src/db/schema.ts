...
// Объявление вариации вариаций на уровне appearance: какие вариации открывает эта пара
// (компонент, стиль), в каком порядке и что у вариации по умолчанию.
//
// Уровень выбран по данным корпуса, а не по удобству: порядок вариаций у 18 компонентов из 70
// не сводится к одной последовательности между стилями, состав вариаций различается между стилями
// у 31 компонента внутри одной ДС, а `defaultValue` одной вариации различается между стилями одного
// компонента в 21 случае. Прежний `styles.is_default` выражал дефолт на уровне (ДС, вариация)
...
export const appearanceVariations = pgTable(
  "appearance_variations",
  {
    ...
    // Снятый дефолт не должен уносить объявление вариации, поэтому set null, а не cascade.
    defaultStyleId: uuid("default_style_id").references(() => styles.id, { onDelete: "set null" }),
    // Роль вариации цветовой схемы: её значения уходят в блок `view`, а не в обычные вариации.
    // Роль объявляет конфигурация, и восстановить её по имени вариации нельзя — в корпусе
    ...
    // Больше одной такой вариации в конфигурации не бывает: замер по двум корпусам исключений
    ...
    /**
     * Тип вариации, объявленный конфигурацией в `bindings[].type`.
     *
     * Хранится отдельно от роли, потому что это независимые факты. В `sdds_serv` они всегда
     * совпадают — вариация схемы объявлена `view` во всех 46 случаях, — и оттого прежде выводились
     * одно из другого. В `sdds_sbcom` 10 вариаций объявлены `enum`, а значения свои держат в `view`:
     * роль говорит, куда уезжают значения, тип — как вариация объявлена.
     *
...
// Объявленные значения вариации, а не использованные. 81 вариация корпуса из 821 объявляет значения,
// которых нет ни в одной вариации, ещё 21 вариация не использует ни одного своего значения:
// `plasma_b2c/badge-clear` объявляет `shape: [default, pilled]`, а переопределения несёт
// только `pilled`. Вывести вариация из строк значений — значит срезать её поверхность.
export const appearanceVariationValues = pgTable(
  "appearance_variation_values",
  {
    ...
    // где-то вариация опущена, где-то порядок обратный порядку объявления. Два способа обойтись
    // без хранения проверены на корпусе и отвергнуты: переименование значений вариаций оставляет
...
// Участники объявленной координаты: по одному стилю на вариация.
...
// уже знает, при каком сочетании вариаций родителя ссылка действует. Хранить его второй раз
...
export const componentStyleReferences = pgTable(
  "component_style_references",
  {
    ...
    // Ровно одна из трёх ссылок заполнена — в зависимости от того, от скольких вариаций родителя
    ...
    // Исходная строка ссылки. Сохраняется, потому что порядок вариаций в ней не выводится
