---
name: sync-spec
description: Sync js/services/db-service/src/openapi/spec.ts with the current DB schema and routes. Use when tables are added/removed, columns change, or route files are added/removed/changed in js/services/db-service/src/routes/api/.
---

Synchronize `js/services/db-service/src/openapi/spec.ts` with the current state of the project.

## Steps

1. Read `js/services/db-service/src/db/schema.ts` — collect all exported table names.
2. Read `js/services/db-service/src/openapi/spec.ts` — collect which tables already have a response schema (`createSelectSchema`) and which are registered in `registerCrud` / `registerPath`. Also collect all paths already registered via `registry.registerPath`.
3. Run `git diff --name-only HEAD` (or check git status) — identify which files in `js/services/db-service/src/routes/api/` were added or modified.
...
5. Read `js/services/db-service/src/validation/schema.ts` — check which Create/Update schemas exist for new tables.
...
- Keep entries in the same order as tables appear in `js/services/db-service/src/db/schema.ts`.
